...
with open('barlist') as f:
	for bar in f: 
		...
		bar_taps = genfromtxt(bar,delimiter=',', names=True, dtype=("|S200", "|S200", "|S200", "|S200", float, "|S200", "|S200", "|S200", float, float))
...
for row in alltaps:
	try:
		newstrrate.append([row[0],row[1],row[2],row[3],str(row[4]),row[5],row[6],row[7],str(row[8]),str(row[9])])
	except NameError:
		newstrrate=[row[0],row[1],row[2],row[3],str(row[4]),row[5],row[6],row[7],str(row[8]),str(row[9])]
