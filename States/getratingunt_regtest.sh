...
cd /home/BesTap/rating_builder/bestap_getter/States
export LANG=C
...
awk '$2 <= n' n="$latestdate" Al_ACT | awk '{print $1}' > thesebars
...
while read j
do
newline=$(awk -v bar="^$j$" -v date="$(date +%s)" '$1 ~ bar { print $1, date}' Al_ACT)
sed -i.bak "s/$j .*/$newline/g" Al_ACT
...
name=$(cat examp | sed 's/ /+/g' | tr '\n' '+' | sed 's/.$//' | sed 's/&/%26/' | sed 's/:/%3A/' | sed 's/#/%23/' |  sed 's/\[/%5B/' | sed 's/\]/%5D/' )
...
join -t',' -o 1.2,2.2,2.3,2.4,2.5,2.6,2.7,2.8,2.9,2.10 sortedbars temp > temp2
